...
resource "aws_instance" "this" {
  ...
  tags_all        = merge(var.default_tags, var.tags)
  volume_tags = var.enable_volume_tags ? merge({ "Name" = var.name }, var.volume_tags) : null

 lifecycle {
  ignore_changes = [
    tags["pam_registered"],
    tags_all["pam_registered"],
...
resource "null_resource" "local_provisioner_pam" {

  
  provisioner "local-exec" {
    command = <<EOT
    sleep  $(( $RANDOM % 10 + 3 ))
    OS="${aws_instance.this[0].tags["OS"]}"
    private_ip="${aws_instance.this[0].private_ip}"
    safe="${aws_instance.this[0].tags["pam_safe"]}"
    
    if [ "$OS" == "Amazon linux 2" ] || [ "$OS" == "RHEL" ]; then
        rst=`curl -i -XGET "http://10.132.80.18:8888/data?tos=ami&ip=$private_ip&safe=$safe"`
      
    elif [ "$OS" == "Ubuntu 18.04" ]  || [[ "$OS" =~ "enterprise-Quant-Avengers" ]]; then
        rst=`curl -i -XGET "http://10.132.80.18:8888/data?tos=ubuntu&ip=$private_ip&safe=$safe"`
        
    else
        rst=`curl -i -XGET "http://10.132.80.18:8888/data?tos=centos&ip=$private_ip&safe=$safe"`
    fi
