...
resource "aws_db_subnet_group" "this" {
  ...
  tags = var.tags
  tags_all = merge(var.tags,var.default_tags)
}
...
resource "aws_rds_cluster" "this" {
  ...
  tags = var.tags
  tags_all = merge(var.tags, var.cluster_tags, var.default_tags)
}
...
resource "aws_rds_cluster_endpoint" "this" {
  ...
  tags = var.tags
  tags_all = merge(var.tags, lookup(each.value, "tags", var.default_tags ,{}))
}
...
resource "aws_security_group" "this" {
  ...
  tags = var.tags
  tags_all = merge(var.tags, var.security_group_tags, var.default_tags,{ Name = var.name })
...
resource "null_resource" "local_provisioner" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      aws rds modify-db-cluster --db-cluster-identifier ${aws_rds_cluster.this[0].id}  --master-user-password $password --apply-immediately  --region ${data.aws_region.current.name}
    EOT
